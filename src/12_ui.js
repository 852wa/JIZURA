...
const S = { project: null, plan: null, audio: null, renderer: new J.Renderer(), playing: false, t: 0, t0: 0, loop: 'all', loopHold: null, tlZoom: 1, need: true, exporting: null, tap: null, slow: false, lineEls: [], curLine: -2 };
...
function tlView() {
  ...
  const z = S.tlZoom || 1;
  const dur = D / z;
  let start = z <= 1 ? 0 : S.t - dur * 0.4;
  start = J.clamp(start, 0, Math.max(0, D - dur));
  return { start, end: start + dur, dur, D };
}
function syncTlZoom() {
...
function setTlZoom(next) {
  S.tlZoom = J.clamp(next, TL_ZOOM_MIN, TL_ZOOM_MAX);
  tlViewHold = null;
  syncTlZoom();
  S.need = true;
}
