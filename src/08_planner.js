...
// Keep effect ownership without adding editor-only metadata to exported plans.
const eventCuts = new WeakMap();
...
J.plan = (project, audio) => {
  ...
  parsed.lines.forEach((ln, li) => {
    ...
    units.forEach((u, k) => {
      ...
      for (let i = evMark; i < plan.events.length; i++) eventCuts.set(plan.events[i], cut);
    });
...
J.lineSnapshot = (plan, li) => {
  ...
  for (const e of plan.events) {
    const owner = eventCuts.get(e);
    let k = owner ? cuts.indexOf(owner) : -1;
    // [start − 0.25, next start − 0.25): an accent just before the following cut belongs to that cut
    // Only use the old timing heuristic for plans that were not generated in this session.
    if (!owner) for (let j = 0; j < cuts.length; j++) { const until = j < cuts.length - 1 ? cuts[j + 1].start - 0.25 : cuts[j].end - 0.3; if (e.t >= cuts[j].start - 0.25 && e.t < until) k = j; }
