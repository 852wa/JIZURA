...
- 曲を読み込むと拍を検出してカットを合わせます。タップでの手動同期のほか、自分の PC で動かす Whisper（音声認識）で各行の開始を自動で合わせることもできます（[歌詞の自動同期](docs/LYRICS_SYNC.md)）。
...
2. **曲とタイミング**
   ...
   - 「自動で同期」は、自分の PC で動かす Whisper（音声認識）が聞き取った時刻に、各行の開始を合わせます。歌詞の文字はそのまま使い、曲は自分の PC 以外には送りません。サーバーの起動方法と仕組みは [docs/LYRICS_SYNC.md](docs/LYRICS_SYNC.md) を参照してください。
3. **構成を選ぶ**
   - 「シャッフル」で全体を再抽選します。
