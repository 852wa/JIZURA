...
- インストール不要。歌詞・曲・書き出し・AI文字起こしはすべてブラウザの中で処理され、音声は文字起こしAPIなどへ送信されません（通常は使用する Google Fonts、AI機能を使うときだけ実行ライブラリとモデルを外部から読み込みます）。
...
- **AIで歌詞を文字起こし**：読み込んだ原曲を歌声向けに音量・帯域調整してから、Whisper Base（高精度）またはTiny（高速）で解析します。開始時刻つきのLRC形式をプレビューしてから歌詞欄へ反映できます（WebGPU優先、WASMへ自動フォールバック）。
...
2. **曲とタイミング**
   ...
   - **AIで歌詞を文字起こし**：言語とモデルを選んで実行すると、原曲を16kHzモノラルへ変換し、低域・高域の整理、軽いダイナミック圧縮、音量正規化を行ってからWhisperが解析します。初期値のBaseはTinyより高精度ですが、ダウンロード量・処理時間・メモリ使用量が増えます。初回だけ選択したモデルをダウンロードし、以後はブラウザキャッシュを再利用します。結果は `[00:12.40]歌詞` の形式で別欄に表示され、内容を修正してから「歌詞欄に反映」を押したときだけ現在の歌詞と置き換わります。歌の認識結果は誤ることがあるため、反映後も歌詞欄や行リストで修正してください。
