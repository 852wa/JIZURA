...
BODY = {
    ...
    'aria-label="AI文字起こし結果"': 'aria-label="AI transcription result"',
    '<button id="btnApplyWhisper" class="primary">歌詞欄に反映</button>': '<button id="btnApplyWhisper" class="primary">Apply to lyrics</button>',
    '文字起こしと音声調整はブラウザ内で処理されます。音声ファイルは文字起こしAPIへ送信されません。': 'Transcription and audio adjustment run in your browser. Your audio file is not sent to a transcription API.',
    'AI文字起こし：Transformers.js / Whisper Tiny・Base（Apache License 2.0）': 'AI transcription: Transformers.js / Whisper Tiny and Base (Apache License 2.0)',
...
UI = {
    ...
    "'AI機能を準備中…'": "'Preparing AI tools…'",
    "`モデル準備中（${info.device === 'webgpu' ? 'WebGPU' : 'WASM'}）…`": "`Preparing model (${info.device === 'webgpu' ? 'WebGPU' : 'WASM'})…`",
    "'モデルダウンロード中…'": "'Downloading model…'",
    '`モデルダウンロード中 ${Math.round(info.progress)}%`': '`Downloading model ${Math.round(info.progress)}%`',
    "'WebGPU非対応のためWASMで実行します。'": "'WebGPU is unavailable; using WASM.'",
    "'WebGPUを利用できなかったためWASMへ切り替えます。'": "'WebGPU could not start; switching to WASM.'",
    "`モデル準備完了（${info.device === 'webgpu' ? 'WebGPU' : 'WASM'}）`": "`Model ready (${info.device === 'webgpu' ? 'WebGPU' : 'WASM'})`",
    "'音声解析中（歌声を拾いやすく調整・16kHzモノラル化）…'": "'Preparing audio (vocal-sensitive adjustment and 16 kHz mono)…'",
    "'文字起こし中…'": "'Transcribing…'",
    "'文字起こし完了'": "'Transcription complete'",
    "'ネットワークに接続されておらず、AIモデルのキャッシュも見つかりません。初回はネットワーク接続が必要です。'": "'You are offline and the AI model is not cached. The first use requires a network connection.'",
    "'メモリが不足しました。他のタブを閉じるか、短い音声で試してください。'": "'Not enough memory. Close other tabs or try shorter audio.'",
    "'AIモデルをダウンロードできませんでした。ネットワーク接続を確認してください。'": "'Could not download the AI model. Check your network connection.'",
    "'先に曲を読み込んでください。'": "'Load a song first.'",
    "'この音声形式を解析できませんでした。MP3・WAV・M4Aなどで試してください。'": "'This audio format could not be analyzed. Try MP3, WAV or M4A.'",
    "'AIモデルを読み込めませんでした。ページを再読み込みして、もう一度試してください。'": "'Could not load the AI model. Reload the page and try again.'",
    "'歌詞を認識できませんでした。音量や音声ファイルを確認してください。'": "'No lyrics were recognized. Check the volume and audio file.'",
    "'文字起こしに失敗しました。音声やブラウザのメモリを確認して、もう一度試してください。'": "'Transcription failed. Check the audio and browser memory, then try again.'",
    ...
    "'歌詞欄に反映しました。必要に応じて歌詞を修正してください。'": "'Applied to the lyrics. Edit the text as needed.'",
